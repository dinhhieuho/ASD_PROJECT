...
//        rowdata = new Object[8];
	...
	void JButtonNewCCAC_actionPerformed(java.awt.event.ActionEvent event) {
		...
		if (newaccount) {
			...
			rowdata[4] = amountDeposit==null?"0.0":amountDeposit;
//			
			model.addRow(rowdata);
			JTable1.getSelectionModel().setAnchorSelectionIndex(-1);
			...
			try {
				...
				if (accountType == "Gold") {
					...
					if (accountType == "Silver") {
						CcardAccount account = cardFactory.createSilverAccount(ccnumber);		
						customer.addAccount(account);
					} else {
						CcardAccount account = cardFactory.createBronzeAccount(ccnumber);
						customer.addAccount(account);
					}
				}
	...
	void JButtonDeposit_actionPerformed(java.awt.event.ActionEvent event) {
		...
		if (selection >= 0) {
			...
			DepositDialog dep = new DepositDialog(this, ccnumber);
