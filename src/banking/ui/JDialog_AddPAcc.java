...
public class JDialog_AddPAcc extends javax.swing.JDialog {
	...
	private BankFactory customerFactory;
	private banking.controller.Controller controller;
	private JtableViewObservable<TableView> subject;

	public JDialog_AddPAcc(BankFactory customerFactory, banking.controller.Controller controller, JtableViewObservable<TableView> subject) {
		...
		JButton_OK.setActionCommand("personaccount");
	...
	void JButtonOK_actionPerformed(java.awt.event.ActionEvent event) {
		...
		String dob = JTextField_BD.getText();
		...
		if (!name.isEmpty() && !street.isEmpty() && !city.isEmpty() && !state.isEmpty() && !zip.isEmpty()
			...
			try {
				CustomerDefault customer = customerFactory.createCustomer(name, street, city, state, Integer.parseInt(zip),
						email, accountNumber, acctype, dob);
